...
/**
 * This object is used in conjunction with a DragDropPresenter. It handles all
 * drag events generated during a drag-drop operation. When a drag starts, it
 * creates a special view (a DragView) that moves around the screen until the
 * user ends the drag. As feedback to the user, this object causes the device to
 * vibrate as the drag begins. It interacts with other objects through methods
 ...
 */
	...
	public boolean onDrag(View v, DragEvent event) {
		...
		case DragEvent.ACTION_DRAG_STARTED:
			...
			// At the start of a drag, all drop targets must say they are
			...
			if (isDragSource) {
				// The view continues to see drag events if it is the source of
				// the current drag
		...
		case DragEvent.ACTION_DROP:
			...
			// "Drag dropped. mDragSource drawable: " + ic2.getDrawable ());
			if (isDropTarget) {
				if (target.allowDrop(mDragSource)) {
					...
					mDropSuccess = true;
					/*
					 * if (mDragSource != null) { mDragSource.onDropCompleted
					 * (mDropTarget, mDropSuccess); }
					 */
				}
				eventResult = true;
			} else
